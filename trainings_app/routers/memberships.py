...
async def get_memberships(access_level: Annotated[Optional[str], Query(description="Filter by access level")] = None,
                          repo=Depends(get_repo(MembershipRepository))) -> list[GetMembership]:
...
async def get_membership(membership_id: Annotated[int, Path(gt=0)],
...
async def create_membership(membership_obj: CreateMembership,
                            repo=Depends(get_repo(MembershipRepository))) -> GetMembership:
...
async def delete_membership(membership_id: Annotated[int, Path(gt=0)],
                            repo=Depends(get_repo(MembershipRepository))) -> GetMembership:
...
async def put_membership(membership_id: Annotated[int, Path(gt=0)],
